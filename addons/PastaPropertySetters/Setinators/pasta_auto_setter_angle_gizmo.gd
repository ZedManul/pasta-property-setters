@tool
class_name PastaAutoSetterAngleGizmo
extends Node2D
## [Not intended for access]


@onready var _visualizee: PastaAutoSetterAngle = get_parent()
...
func _draw() -> void:
	if _visualizee is PastaAutoSetterAngle:
		if (
			_visualizee.target_node is Node2D 
...
func _draw_border_star() -> void:
	var node: Bone2D = _visualizee.target_node
